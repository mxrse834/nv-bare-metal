...
static int probe(struct pci_dev *dev, const struct pci_device_id *id)
{
    ...
    struct pvt_dev *pvt  =  kmalloc(sizeof(struct pvt_dev),GFP_KERNEL);
    if(!pvt){printk(KERN_ERR"nv_probe: kalloc failed : driver_private_data notsaveds");err = -ENOMEM;goto unmap;}
    ...
    iowrite32((1 << 16)|(1<<8)| p1,bar0 + 0x200);
    pfifo_intr = ioread32(bar0 + 0x2100);
    p1 = ioread32(bar0 + 0x200);
    printk(KERN_INFO "nv_probe: PMC_ID = 0x%08x\n", p0);
    printk(KERN_INFO "nv_probe: PMC_ENABLE = 0x%08x\n", p1);
    printk(KERN_INFO "nv_probe: PFIFO_INTR = 0x%08x\n",pfifo_intr);
    ...
    if(!pb_cpu){printk(KERN_ERR "nv_probe: dma_alloc_coherent failed\n"); err = -ENOMEM; goto free_mem;}
    printk(KERN_INFO "nv_probe: DMA address 0x%llx, CPU virtual %p\n", pb_dma, pb_cpu);    
    pvt->pb_dma = pb_dma;
    pvt->pb_cpu = pb_cpu;
    pvt->bar0 = bar0;
...
static void remove(struct pci_dev *dev)
{
    struct pvt_dev *pvt = pci_get_drvdata(dev);
    dma_free_coherent(&dev->dev,4096,pvt->pb_cpu,pvt->pb_dma);
    pci_iounmap(dev,pvt->bar0);
    kfree(pvt);
    pci_release_regions(dev);
    pci_disable_device(dev);
